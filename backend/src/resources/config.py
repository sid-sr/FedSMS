...
class Config(Resource):
    def get(self):
      ...
      decimal_encoded = ast.literal_eval((json.dumps(current_config['Item'], cls=DecimalEncoder)))
    ...
    def put(self):
      ...
      current_config = ConfigTable.get_item(Key = {'id': 0}) 
      decimal_encoded = ast.literal_eval((json.dumps(current_config['Item'], cls=DecimalEncoder))) 
      if(decimal_encoded['modelIndex']==0): #if there is a round in progress, config info cannot be updated
